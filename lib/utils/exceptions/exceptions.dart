...
//--Factory: Dio and Backend Errors ke liye
  factory TExceptions.fromDioError(dynamic e) {
    String errorMessage = 'Something went wrong. Please try again.';

    if (e is DioException) {
      if (e.response != null && e.response?.data != null) {
        // Backend 'detail' bhej raha hai (e.g. Invalid email or password)
        ...
        if (data is Map) {
          ...
          if (detail is String) {
            ...
            if (detail['code'] != null) {
              final mappedException = TExceptions.fromCode(detail['code'].toString());
              if (mappedException.message != const TExceptions().message) {
                return mappedException;
              }
            }
            errorMessage = detail['message'] ?? detail['error'] ?? errorMessage;
          } else if (detail is List && detail.isNotEmpty) {
            // Validation errors list se pehla message nikal lo
        ...
        errorMessage = 'Server is not responding. Try later.';
      }
    } else {
      errorMessage = e.toString();
    }

    return TExceptions(errorMessage);
  }
